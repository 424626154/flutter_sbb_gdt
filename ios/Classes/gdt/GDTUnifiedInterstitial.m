...
- (instancetype)init:posId binaryMessenger:(NSObject<FlutterBinaryMessenger> *)messenger
{
    if([super init]) {
        self->posId = posId;
        NSString *channelName = [GDTUnifiedInterstitial getChannelName:posId];
...
- (void)loadAd{
    ...
    self.ad = [[GDTUnifiedInterstitialAd alloc] initWithAppId:[GDTConfig sharedInstance].appid placementId:posId];
