...
class SplashListener {
  ...
  Future show () async {
    ...
    print('SplashListener show ${rets}');
    if (rets["channel_name"]?.isNotEmpty ?? false) {
...
class RewardVideoListener {
  ...
  StreamController<String> _onRewardStream = StreamController.broadcast();

  Stream<String> get getRewardStream => _onRewardStream.stream;
  ...
  RewardVideoListener(this.appId,this.posId);

  Future show () async {
    print('RewardVideoListener show ');
    final rets = await FlutterGdtPlugin.showRewardVideo(appId,posId);
    print('RewardVideoListener show rets: ${rets}');
    if (rets["channel_name"]?.isNotEmpty ?? false) {
      _channel = MethodChannel(rets["channel_name"]);
      _channel.setMethodCallHandler(_handleMessages);
    }
...
class BannerListener {
  ...
  void onNoAD (int code, String msg) {
    print("onNoAD code:${code} msg:${msg}");
    print("onNoAD :${_onNoADStream}");
    _onNoADStream.add(AdsError(code: code, msg: msg));
  }
